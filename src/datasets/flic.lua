...
function FlicDataset:get(i, scaleFactor)
  local scaleFactor = scaleFactor or 1
  local img = image.load(paths.concat('data/flic/images', 
  ...
  return {
    ...
    width = img:size(3),
    height = img:size(2),
    imgPath = paths.concat('data/flic/images', 
        self.imageInfo.data['images'][i]),
...
function FlicDataset:augmentation(input, level2gt, level2isEmpty)
