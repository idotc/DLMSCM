...
function MpiiLeedsDataset:get(i, scaleFactor)
  ...
  local scale = self.imageInfo.labels['scale'][i]*(410/256)*scaleFactor
  -- (x,y)
  ...
  level2isEmpty[1] = {}
  for i = 1,nParts[1] do
    local j = newPoints[children[1][i]]
  ...
  if nSemanticLevels >= 2 then
    ...
    for i = 1,nParts[2] do 
      ...
      if not (level2isEmpty[1][idx1] or level2isEmpty[1][idx2]) then
        local j1, j2 = newPoints[children[1][idx1]], newPoints[children[1][idx2]]
...
function MpiiLeedsDataset:augmentation(input, level2gt, level2isEmpty)
