...
function visualizer.drawSkeleton(input, coords, hms)
    ...
    for i = 1,#pairRef1 do
        if hms and hms[pairRef1[i][1]]:mean() > actThresh and hms[pairRef1[i][2]]:mean() > actThresh then
            -- Set appropriate line color
            local color
            if partColor[pairRef1[i][1]] == 1 then color = {0,.3,1}
            elseif partColor[pairRef1[i][1]] == 2 then color = {1,.3,0}
            elseif partColor[pairRef1[i][1]] == 3 then color = {0,0,1}
            elseif partColor[pairRef1[i][1]] == 4 then color = {1,0,0}
            else color = {.7,0,.7} end

            -- Draw line
            im = drawLine(im, coords[pairRef1[i][1]], coords[pairRef1[i][2]], 4, color, 0)
        end
    end
...
function visualizer.drawSkeleton_mpii(input, coords, hms)
    ...
    for i = 1,#pairRef do
        if hms and hms[pairRef[i][1]]:mean() > actThresh and hms[pairRef[i][2]]:mean() > actThresh then
            ...
            -- Draw line
            im = drawLine(im, coords[pairRef[i][1]], coords[pairRef[i][2]], 8, color, 0)
        end
    end
...
function visualizer.drawOutput(input, hms, coords)
    ...
    local im = input
    if coords then
        im = visualizer.drawSkeleton(input, coords, hms)
    end
